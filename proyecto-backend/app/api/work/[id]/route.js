  ...
  try {
    ...
    for (const photo of photos) {
      if (photo instanceof Object) {
        ...
        const workImagePath = `/Users/stephanievalderramacarmona/Library/Mobile Documents/com~apple~CloudDocs/Documents/Johan_Flowcode/fullstack_factoriaF5/repositorios_github/Proyecto_Backend/proyecto-backend/public/upload/${photo.name}`;

        // Write the buffer to the filessystem
        await writeFile(workImagePath, buffer);

        // Store the file path in an array
        workPhotoPaths.push(`/upload/${photo.name}`);
      } else {
